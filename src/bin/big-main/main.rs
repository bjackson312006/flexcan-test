...
use embassy_mcxa::flexcan::classic::frame::{Frame, StandardId, ExtendedId};
...
const EXAMPLE_MESSAGE_TWO: StandardId = StandardId::new(0xFA).unwrap();
const EXAMPLE_MESSAGE_THREE: ExtendedId = ExtendedId::new(0x1232).unwrap();
const EXAMPLE_MESSAGE_FOUR: ExtendedId = ExtendedId::new(0xFAF).unwrap();
...
async fn can0_rx_task(rx0: FlexCanRx<'static>) {
    loop {
        ...
        defmt::info!("CAN0 RX id={:?} len={} payload={}", frame.id(), frame.dlc(), u64::from_be_bytes(frame.data().try_into().expect("Slice must be 8 bytes.")));
    }
}
...
async fn can1_tx_task(mut tx1: FlexCanTx<'static>) {
    loop {
        let frame = Frame::new(EXAMPLE_MESSAGE_TWO, &Instant::now().as_millis().to_be_bytes()).unwrap();
        let frame2 = Frame::new(EXAMPLE_MESSAGE_THREE, &Instant::now().as_millis().to_be_bytes()).unwrap();
...
async fn main(spawner: Spawner) {
    ...
    let can0 = FlexCan::new(p.CAN0, p.P1_11, p.P1_2, FlexCanConfig {
        filters: filters!(
            Filter::Standard(EXAMPLE_MESSAGE_ONE), Filter::Standard(EXAMPLE_MESSAGE_TWO), Filter::Extended(EXAMPLE_MESSAGE_THREE), Filter::Extended(EXAMPLE_MESSAGE_FOUR),
        ),
        bitrate: 1_000_000,
